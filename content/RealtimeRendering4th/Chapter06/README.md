- [第6章 纹理](#第6章-纹理)
  - [1 纹理管线](#1-纹理管线)
  - [2 图像贴图(Image Texturing)](#2-图像贴图image-texturing)
  - [3 程序纹理(Procedural Texturing)](#3-程序纹理procedural-texturing)
  - [4 纹理动画](#4-纹理动画)
...
![](Media/Equation_06_14.png)

* parallax occlusion mapping(POM), 关键思路：沿着投影方向测试固定数量的高度场纹理样本，当视线趋于水平时，采样更多的样本。沿着视线方向，计算其三维空间位置，将该位置变换至切线空间，确定该位置高于还是低于高度场相应位置的高度。一旦发现某样本计算的高度数据低于该样本的高度场高度，且之前的样本都高于高度场高度时，则发现了相应的交点。而后使用该位置的法线，颜色， 纹理进行着色。查找最近相交点的方法，割线方法(secant method), 二进制搜索等. 为了提高性能和采样精度，研究人员提出很多方法。
* 注意POM的很多方法有个限制，受被渲染模型的轮廓影响， 如下图, 上半部分没使用 POM， 下半部分使用 POM 实现， 但轮廓部分失真。

![](Media/Figure_06_15.png)

* 可以通过向外挤出一个壳来解决， 这样就可以在外面的一层上进行片段处理。效果可以看下图。

![](Media/Figure_06_16.png)

## 9 纹理光照
* 纹理可以增加光照的多样性，更复杂的光强分布，聚光灯效果。可以实现其他形状的聚光灯， 带图案的光源，如下图

![](Media/Figure_06_17.png)

* cube map 可以调节全方向光照的光强。一维纹理可以用来定义任何距离衰减函数。结合二位衰减映射(map)，可以实现复杂的体积光照模式。还有种就是通过三维纹理控制灯光的衰减。对于对称的光照，可以保存其八分之一的数据(一个象限内)即可。